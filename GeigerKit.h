...
//volatile byte IR_Dev = 0;             // returns IR device code received - not used

// IR receiver defines . . .
#define POWER   100                     // define keytypes
#define MENU    101
#define TEST    102
#define PLUS    103
#define BACK    104
...
boolean tonePotMode;                    // controls whether tone sensitivity is set using a potentiometer
...
unsigned int potVal;                    // holds pot value if tone pot used
static unsigned int lastPotVal = 0;     // used for hysteresis if tone pot used
...
int  adc_key_val[5] = {
  20, 45, 100, 140, 200
};              // ADC return must be UNDER these values
#endif
...
byte cpmIcon[8] = {
  0x18, 0x11, 0x1A, 0x04, 0x00, 0x1B, 0x15, 0x11
}; // "CPM" icon
byte usvIcon[8] = {
  0x14, 0x1C, 0x00, 0x18, 0x10, 0x0D, 0x1A, 0x00
}; // "uSv" icon
byte oneIcon[8] = {
  0x0C, 0x04, 0x04, 0x0E, 0x00, 0x1B, 0x15, 0x11
}; // 1 minute icon
byte tenIcon[8] = {
  0x17, 0x15, 0x17, 0x00, 0x00, 0x1B, 0x15, 0x11
}; // 10 minute icon
byte cpm2Icon[8] = {
  0x18, 0x11, 0x1A, 0x04, 0x00, 0x1B, 0x15, 0x11
}; // "CPM" icon again (Serial.write(0) no work
#else
