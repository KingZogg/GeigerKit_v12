/* GeigerKit default sketch  FOR v%.6  [20442 - 63% / 1146 - 55% bytes v1.6.11]   bHogan 8/28/16
 * Compiles under Arduino IDE v1.6.5, and v1.6.11 - Board = Uno
 ...
 *
 * FEATURES:
 * The features in this release are described on the DIYGeigerCounter web site:
 ...
 *
 * SETUP: See GeigerKit.h for pin mapping
 *
 * TODO:
 * - remove serial header - DOC
 * - Fix "Pot" setting in MENU_TONE_SENS for better operation.
 * - Update B5 software features page with new wiring diagram.
 * - TEST TEST TEST
 *
...
#if (ANDROID)
  Android.registerFunction(androidInput, 'o'); // calls androidInput() when new input from droid
#endif
...
void loop() {
  static boolean scalerDispOn = false;  // true when SW_1 on, and in continuous count mode
...
void DispCounts(unsigned long dcnt) {   // calc and display predicted CPM & uSv/h
  ...
  if (!PiezoOn) {                       // put "*" in the corner of the disply if MUTE is on
    lcd.setCursor(15, 1);
    lcd.print("*");
    lcd.setCursor(0, 1);
  }
...
void setAlarm() { // RECURSIVE FUNCTION to change alarm set point when button repeatedly pushed
